...
class MaterializedChainState:
    ...
    def ref_caller(self, index: int, call_data: bool):
        try:
...
def main():
    ...
    parser.add_argument('--call_data', action='store_true', default=False, help='Flag to indicate whether --call_data is specified')
    args = parser.parse_args()
    chain_state = MaterializedChainState(url=args.url)

    if args.ref_caller:
        result = chain_state.ref_caller(args.ref_caller, args.call_data)
        print(result)
